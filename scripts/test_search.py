...
def test_pipeline():
    ...
    t0 = time.perf_counter()
    results_dune = rec.recommend_similar_to_book("Dune", top_k=5)
    lat = (time.perf_counter() - t0) * 1000.0
    print(f"Target: Dune (Latency: {lat:.1f}ms)")
    for i, r in enumerate(results_dune, 1):
        print(f"  {i}. {r['title']} by {r['author']} | Match: {r['similarity_score']*100:.1f}% | Genres: {r['genres']}")

    print("\n--- TEST 3: Book-to-Book Similarity (Target: Dracula) ---")
    results_drac = rec.recommend_similar_to_book("Dracula", top_k=5)
    for i, r in enumerate(results_drac, 1):
        print(f"  {i}. {r['title']} by {r['author']} | Match: {r['similarity_score']*100:.1f}% | Genres: {r['genres']}")
